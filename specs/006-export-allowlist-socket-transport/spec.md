...
### Session 2026-08-12 — achados da primeira implantação isolada de verdade

A topologia dividida foi montada pela primeira vez com isolamento real, e não apenas com o transporte de rede ativo: máquina virtual Lima sobre QEMU, sem nenhum sistema de arquivos do hospedeiro montado, harness dentro dela, servidor e caso do lado de fora. A separação se verificou — nenhum mount de origem no hóspede, pasta do caso inalcançável — e **três coisas quebraram que a suíte inteira não via**, porque as três só existem quando os dois lados deixam de compartilhar disco.

- Q: O agente leu o caminho do caso, notou que estava em Linux, concluiu que o caso havia sumido e saiu procurando no sistema de arquivos do **próprio ambiente isolado**. Nunca chamou a ferramenta de abertura. Isso é defeito de implementação ou falta requisito? → A: **Falta requisito, e ele estava a um documento de distância do lugar certo.** FR-025 exige que a **documentação de instalação** cubra a topologia dividida, e ela cobre — inclusive dizendo que o artefato exportado nasce do lado do servidor. Só que esse documento é lido pelo perito, não pelo agente. A orientação que o agente segue dizia a mesma coisa **apenas sobre o destino de exportação**, e nada sobre o caminho do caso. Enquanto harness e servidor dividiam disco, a omissão era inofensiva; a topologia dividida a converte em falha silenciosa, e silenciosa é a palavra que importa: um agente que procura no disco errado **não produz erro nenhum** — ele simplesmente nunca chama a ferramenta que teria funcionado, e o perito vê uma sessão que "não achou o caso". **FR-036** e **SC-017** criados.
- Q: Para o harness falar com o servidor, o ambiente isolado precisou de um runtime Java inteiro, porque o intermediário é um programa Java. Isso é escolha de implementação ou o requisito deveria dizer algo? → A: **Deveria dizer.** O intermediário é uma linha de handshake e dois bombeamentos de bytes; FR-035 governa o que ele precisa **fazer** e nada dizia sobre o que ele pode **exigir**. E o que ele exige incide justamente sobre o ambiente cujo valor inteiro é ser pequeno o bastante para se auditar: instalar um segundo runtime lá dentro, para mantê-lo atualizado e para ampliar a superfície, é caro na moeda errada. Medido na implantação: cinco jars e um JRE contra um script de cinco quilobytes sobre o interpretador que a imagem já traz. **FR-037** e **SC-018** criados.
- Q: Durante a implantação, uma linha de log apareceu no meio do fluxo do protocolo. As duas configurações de log distribuídas, e o próprio padrão da biblioteca quando nenhuma é indicada, escrevem em stdout — que nesses processos **é o canal do protocolo**. Isso é defeito ou configuração? → A: **É requisito que falta.** A invariante "nada escreve em stdout" já vigorava no código do módulo e é respeitada por ele; o que a contradiz vem de fora, do arquivo de log que a instalação carrega. A correção existia — passar uma configuração que aponte para stderr —, mas dependia de o perito lembrar de uma opção de linha de comando que os próprios comandos documentados **não traziam**. E a falha não é ruidosa: ela corrompe a primeira sessão que registrar qualquer coisa, e o sintoma se parece com defeito de protocolo do servidor, não com ajuste de log. Uma garantia que depende de alguém lembrar não é garantia. **FR-038** e **SC-019** criados.
...
- **FR-037**: O intermediário entre harness e servidor MUST poder ser implantado em um ambiente isolado **sem exigir a instalação de um runtime de linguagem só para ele**. O ambiente isolado existe para ser pequeno o bastante para se auditar, e cada runtime acrescentado ali é superfície nova e mais uma coisa a manter atualizada — custo pago no lugar errado, já que o intermediário não abre caso, não lê evidência e não grava artefato: ele move bytes. Oferecer mais de uma implementação equivalente MUST ser aceitável, desde que ao menos uma delas seja satisfeita pelo que ambientes isolados usuais já trazem, e desde que todas falem o mesmo protocolo com o mesmo servidor.
...
- **FR-036**: A orientação que **o agente** segue MUST estabelecer que todo caminho da superfície de ferramentas — o caso que se abre e o destino para onde se exporta — pertence ao sistema de arquivos do servidor, e que a única forma de saber se um caminho de caso presta é submetê-lo à ferramenta de abertura. Verificar caminho de caso por inspeção do sistema de arquivos local MUST ser desaconselhado explicitamente. Não basta que a **documentação de instalação** de FR-025 diga isso: aquele documento é lido por quem implanta, e este requisito trata do que o agente sabe em tempo de execução. A falha que ele previne é silenciosa — um agente que conclui que o caso sumiu porque o caminho não existe na máquina dele não chama ferramenta alguma e portanto não gera erro que denuncie o engano.
- **FR-038**: A configuração de registro distribuída com a instalação MUST NOT escrever no canal que transporta o protocolo, e a instalação MUST oferecer uma configuração que garanta isso para o servidor e para o intermediário. Os comandos de execução publicados na documentação de FR-025 MUST empregá-la. A invariante interna do módulo — nada escreve na saída padrão — MUST NOT depender, para valer de fato, de o operador lembrar de uma opção de linha de comando: uma linha de registro no meio do fluxo corrompe a sessão e se apresenta como defeito de protocolo do servidor, não como ajuste de registro.
...
- **SC-017**: Um agente que recebe um caminho de caso escrito na convenção de outro sistema operacional — ambiente isolado em um, servidor em outro — submete esse caminho à ferramenta de abertura em 100% das execuções, e em nenhuma delas conclui que o caso é inalcançável a partir de inspeção do sistema de arquivos do ambiente isolado.
- **SC-018**: A implantação do intermediário em um ambiente isolado recém-criado, a partir de uma imagem usual de sistema operacional, não exige a instalação de nenhum runtime de linguagem adicional, e o que precisa ser copiado para lá cabe em menos de um megabyte.
- **SC-019**: Sobre uma bateria de execuções do servidor e do intermediário com os comandos exatamente como publicados na documentação, **nenhum byte que não seja do protocolo aparece na saída padrão**, em 100% das execuções, incluindo as que produzem registro de nível informativo ou de erro.
