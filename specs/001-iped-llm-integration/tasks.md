...
- [X] T103 [US1] Abrir o repositório de previews do caso ao abri-lo e liberá-lo ao fechá-lo: `CasePool.acquire` chama `PreviewRepositoryManager.configureReadOnly(moduleDir)` e `release`/`close` chamam `PreviewRepositoryManager.close(moduleDir)`, e `ContentAccess.extractText` passa a declarar falha de servidor como falha de servidor (FR-085, FR-086) — **concluída em 2026-09-05**, com `integration/PreviewBackedContentTest` 2 de 2 na bancada, nenhum pulado, e a suíte inteira sem regressão. Veio de campo pela terceira vez, mesmo padrão: o `UICaseDataLoader` faz a chamada ao abrir caso — quatro linhas antes do `installCustomSignatures` da T102 — e o servidor não fazia; item decodificado sem trecho de evidência próprio recebe `PreviewInputStreamFactory` do `IndexItem` e seus únicos bytes estão no `previews.mv.db`. O item reportado (`application/x-ufed-instantmessage`) passa a extrair pelo `UfedMessageParser`. **O seletor do teste custou três tentativas e é a parte que merece atenção**: `hasPreview:true` são 1,7 milhão de itens quase todos com bytes próprios, e a primeira versão do teste **passou com o conserto removido**; ampliar para `OR category:"Chats"` devolveu outra população inteira. Só `category:"Instant Messages"` + ausência de tamanho isola a classe (75 de 75), e o controle negativo foi rodado até falhar de verdade antes de aceitar o teste
- [X] T102 [US1] Registrar os media types do IPED no arranque: `McpServerMain.bootstrapConfiguration` chama `SignatureTask.installCustomSignatures()` depois de `loadConfigurables` e **antes de existir parser no processo** (FR-083) — **concluída em 2026-09-04**. Encontrada ao perguntar por que a UI acerta: o `UICaseDataLoader` faz essa chamada antes de criar seu parser, e o `CustomSignatures.xml` declara `application/x-whatsapp-chat` como `sub-class-of text/html`. Medido em JVM limpa: com o registro, `hasSpecificParser` passa de `false` para `true`, o parser folha de `RawStringParser` para `HtmlParser`, e o artefato `[UnNamed]-0` que sobrava no início do texto **desaparece**; sem mudança em PDF, JPEG, `text/plain` nem na mensagem decodificada. A T101 tratava o sintoma — o registro é a causa. **Armadilha registrada**: a chamada só define propriedade de sistema lida na construção do registro de mimes, então chamada tarde ela não faz nada e não avisa; foi assim que o primeiro teste desta hipótese a "refutou"
- [X] T101 [US1] Texto de item de conversa: `ContentAccess.extractText` só fixa `Indexer-Content-Type` quando `StandardParser.hasSpecificParser` confirma parser para ele, e `ContentAccess.noText` deriva o motivo de `isDecodedData`, media type, `isTimedOut` e dos campos de metadado do próprio item (FR-083, FR-084) — **concluída em 2026-09-04**, com `integration/ItemTextTest` 4 de 4 na bancada, nenhum pulado, e a asserção nova do `AvailabilityTest` exigindo motivo que nomeie algo verdadeiro daquele item. Veio de campo: o texto de um chat voltava o HTML do preview e o de uma mensagem voltava "pode ser binário, não parseado ou cifrado" — as duas com a mesma causa, o fallback `RawStringParser` que **nunca falha**. Medido antes de propor: 3 receitas de parse lado a lado sobre o mesmo item, e depois 5 classes de item (chat, mensagem, PDF, JPEG, `text/plain`) para confirmar que só o que estava errado mudou. A suíte seleciona o chat pelo **vocabulário do caso** (agregação por `contentType` dentro das categorias de conversa), nunca por decoder nomeado
- [X] T099 [US1] Testes do custo de uma página: `unit/MatchAllQueryTest` sobre o reconhecimento de "todo item" (as duas grafias, e o que **não** é reconhecido, que importa mais), `unit/CursorPaginationTest.theCollectorAlreadyKnowsTheWholeTotalOnEveryPage` fixando a semântica do Lucene de que a remoção do `count()` depende — se um upgrade mudar `totalHits` sob `totalHitsThreshold = Integer.MAX_VALUE`, `total_matches` passaria a significar outra coisa em silêncio, e é aqui que tem de falhar — e `integration/SearchTotalsTest` para o contrato de resultado (FR-012, FR-079, FR-081, FR-082) — **concluída em 2026-09-04**, 5 de 5 na bancada, nenhum pulado: o caso tem marcador e o corte por `timeout_ms = 1` disparou de verdade, então a asserção de piso, de `next_cursor` ausente e de motivo declarado rodou contra índice real
- [X] T100 [US1] Uma avaliação por página em `PagedSearcher`: total lido de `TopDocs.totalHits` em vez de um `searcher.count()` separado, reconhecimento de `*`/`*:*` como match-all antes do parser, `next_cursor` suprimido em página parcial com motivo, e `query` opcional em `iped_search` quando vem `bookmark` (FR-012, FR-079, FR-081, FR-082) — **concluída em 2026-09-04**. Vinha de campo: `bookmark` + `query: "*"` demorando minutos. O marcador não era a causa — `hasBookmark` é o mesmo teste de bit que a UI faz. Eram o wildcard sobre `name`/`content` com `SCORING_BOOLEAN_REWRITE` (uma cláusula por termo do índice, com o teto de cláusulas em `Integer.MAX_VALUE`, então nada falha e só demora), a consulta rodando **duas** vezes, e o `count()` fora do orçamento de tempo — que é como uma consulta caríssima ignorava o `timeout_ms` pedido
- [X] T097 [US1] Testes da projeção de campos: `unit/FieldSelectionTest` sobre a resolução de nomes (nome do índice, chave publicada pelo servidor, grafia de consulta, diferença só de caixa, nome desconhecido, nome namespaced cortado no colon, `content`, `snippet`) e `integration/FieldProjectionTest` ponta a ponta pelo dispatcher (FR-080) — **concluída em 2026-09-04**, verificada contra caso real do release `4.4.0-SNAPSHOT`: **7 de 7, nenhum pulado**. A suíte de integração é **agnóstica ao conteúdo de propósito** — tira os ids da própria busca e o campo específico do próprio `iped_list_fields` —, para não entrar na lista de suítes que só valem contra o caso da receita. O teste que mais importa é o que compara as duas formas item a item: tamanho número, timestamp instante ISO, flag booleana nas duas, senão quem compara a projeção com a resposta padrão compara coisas diferentes do mesmo item
...
- [X] T098 [US1] Implementar `iped-mcp/src/main/java/iped/mcp/item/FieldSelection.java` e o parâmetro opcional `fields` do `iped_get_items`, com `FieldNames.fromQueryForm` para aceitar a grafia de consulta onde se espera nome (FR-080) — **concluída em 2026-09-04**. A resolução roda **antes** de ler documento e recusa a chamada inteira quando um nome não existe no caso; sem o parâmetro, a resposta é idêntica à anterior, fixado por `theDefaultProjectionIsUnchangedWhenNoFieldsAreAsked`. Duas armadilhas do índice moram aqui: `isRoot` só é gravado quando verdadeiro, então **ausência é `false`** e não indeterminado; e campo binário (`thumbnail`, features) tem `stringValue()` nulo e é declarado em `unavailable` apontando a ferramenta que devolve bytes. O `ToolSchemaTest` precisou de ajuste: exigia literalmente `integer` em todo array e passou a exigir **um tipo escalar declarado**, que é o que o nome e a mensagem do teste sempre disseram
