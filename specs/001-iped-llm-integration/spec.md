...
- Q: Por que o parser certo não roda? → A: **Porque nós fixamos o tipo.** `ParsingTask.fillMetadata` põe o media type do item em `Indexer-Content-Type`, e tipos que o IPED *atribui* a itens decodificados (`application/x-whatsapp-chat`, `message/x-whatsapp-message`) **não têm parser registrado**. O `StandardParser` cai no `RawStringParser`, que devolve os bytes imprimíveis e **nunca falha** — daí o silêncio. Passa a fixar só quando `hasSpecificParser` confirma que há parser; caso contrário, deixa o detector ler os bytes. Medido: o chat sai de `RawStringParser` sobre marcação para `HtmlParser` sobre a conversa, com PDF, JPEG e `text/plain` inalterados. **FR-083 criado.**
...
- **FR-083**: O texto devolvido para um item MUST ser o conteúdo dele, nunca a sua forma de origem. Quando o media type que o item carrega não tiver parser capaz de tratá-lo — o caso dos tipos que o próprio processamento **atribui** a itens decodificados —, o sistema MUST deixar o tipo ser detectado do conteúdo em vez de impor o tipo armazenado, porque impô-lo seleciona o tratamento genérico de último recurso, que devolve os bytes imprimíveis e **nunca falha**: o defeito chega ao perito como texto plausível, não como erro. Quando o tipo que produziu o texto não for o tipo do item, ambos MUST vir declarados, sob pena de o tipo de parsing ser registrado no laudo como se fosse o do item.
