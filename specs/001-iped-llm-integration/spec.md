...
### Session 2026-09-04 — o custo de uma página, vindo de campo

Um chamado de campo demorou minutos: `iped_search` com `bookmark` e `query: "*"`. O marcador não era a causa — o teste de pertinência dele é o mesmo bit que a UI do IPED testa. As causas eram três, e nenhuma estava no lugar onde se olhou primeiro.

- Q: `"*"` e `"*:*"` significam o mesmo para quem escreve. Devem custar o mesmo? → A: **Sim, e o servidor passa a reconhecer os dois.** `*:*` seleciona todo item; `"*"` é wildcard sobre `name` e `content` com `SCORING_BOOLEAN_REWRITE`, ou seja, uma cláusula booleana **por termo do índice** — e o engine levanta o teto de cláusulas para `Integer.MAX_VALUE`, então nada falha, só demora o tamanho do dicionário. O reconhecimento é **estreito de propósito**: só uma expressão que é *apenas* a estrela. Adivinhar em `name:*` ou `* AND x` seria o servidor decidir o que o perito pediu, o que é pior que consulta lenta. O reparo vem declarado em `query_normalized`.
- Q: Por que o agente escreveu `"*"`? → A: **Porque a ferramenta exigia consulta.** Na UI, "me mostre este marcador" é filtro de pertinência, não busca. Exigir `query` obrigava a inventar uma expressão que significasse "tudo", e a que se inventa naturalmente é a caríssima. A armadilha era da nossa superfície. `query` passa a ser opcional quando vem `bookmark`. **FR-081 criado.**
- Q: A contagem exata justifica uma segunda execução da consulta? → A: **Não, e ela nunca foi necessária.** O `TopFieldCollector` roda com `totalHitsThreshold = Integer.MAX_VALUE`, que proíbe término antecipado: `totalHits` já é o total do conjunto, em qualquer página. O `searcher.count()` recalculava um número que estava na mão — e, pior, rodava **fora do orçamento de tempo**, que é como uma consulta caríssima ignorava o `timeout_ms` que o agente pediu. Removido. **FR-082 criado.**
- Q: E quando o orçamento estoura, o total deixa de ser exato. Aceitável? → A: **Aceitável se declarado, e é melhor que a alternativa.** Passa a ser piso — "pelo menos N" —, com `total_matches_exact: false`. Para o uso real do total (decidir se a consulta é ampla demais), piso serve; e o exato-a-qualquer-custo é literalmente o que fazia a chamada pendurar. Note que isso **não** se lê no `TotalHits.relation`, que diz `EQUAL_TO` mesmo com a varredura cortada: o `relation` descreve o teto, não a interrupção.
- Q: Página parcial pode devolver cursor? → A: **Não.** O cursor retoma depois do último item coletado; um acerto que a varredura não alcançou pode ordenar **antes** dessa posição e ficaria de fora desta página e de todas as seguintes, em silêncio. É a lógica do próprio FR-079 — cursor que não se pode confiar que avance é ausência declarada. Passa a vir `next_cursor_omitted` com o motivo.
- Q: `timeout_ms` é garantia de tempo de resposta? → A: **Não, e isso fica escrito.** O `TimeLimitingCollector` consulta o relógio dentro de `collect()`, durante a varredura. A expansão de uma expressão caríssima acontece antes, na montagem da consulta, onde o cronômetro não é consultado. O orçamento limita a varredura, não o plano — por isso a proteção real contra `"*"` é o reconhecimento acima, não o timeout.

### Session 2026-09-04 — texto de item de conversa, vindo de campo

Duas chamadas reais sobre um caso de extração de celular: o texto de uma **mensagem** voltou "nenhum texto pôde ser extraído; pode ser formato binário, não parseado ou cifrado", com o conteúdo dela em `Message-Body`; e o texto de um **chat** voltou o fonte HTML do preview, com o teto de caracteres gasto em `<head>` e num favicon base64. Mesma causa raiz, medida e não suposta.

- Q: Por que o parser certo não roda? → A: **Porque o servidor não registrava os media types do IPED.** Primeiro diagnóstico, e ele estava incompleto: culpou o `Indexer-Content-Type` fixado por `ParsingTask.fillMetadata` e passou a fixá-lo só quando `hasSpecificParser` confirma parser. Funcionava, mas tratava o sintoma. A causa foi encontrada ao perguntar por que a UI acerta: o `UICaseDataLoader` chama **`SignatureTask.installCustomSignatures()`** antes de criar seu parser, e o `conf/CustomSignatures.xml` declara `application/x-whatsapp-chat` como **`sub-class-of text/html`**. Com o registro, o Tika sobe a hierarquia e acha o `HtmlParser`; sem ele, nenhum parser responde pelo tipo e o `StandardParser` cai no `RawStringParser`, que devolve os bytes imprimíveis e **nunca falha** — daí o silêncio. O servidor passa a registrar no arranque. **FR-083 criado.**
- Q: E por que a primeira tentativa de testar essa hipótese a refutou? → A: **Ordem.** `installCustomSignatures` só define uma propriedade de sistema, lida pelo Tika quando ele **constrói** o registro de mimes. Chamada depois de qualquer coisa já ter tocado o Tika, ela não registra nada e **falha em silêncio** — foi assim que o experimento pareceu descartar a hipótese certa. Vale como regra ao mexer aqui: registrar mime é inicialização, não configuração, e tem que preceder o primeiro parser do processo.
- Q: A detecção por `hasSpecificParser` continua? → A: **Sim, como segunda linha.** Com o registro em vigor ela não dispara para os tipos deste caso. O que sobra para ela é tipo que **esta instalação não conhece** — caso processado por IPED mais novo, com decoder cujo tipo o `CustomSignatures.xml` do servidor antecede —, cenário real aqui porque o servidor abre caso de qualquer versão 4.x. E o resultado declara quando ela agiu.
- Q: E o tipo que parseou passa a ser o tipo do item? → A: **Não, e os dois vêm declarados.** O item continua sendo `application/x-whatsapp-chat`; quem extraiu foi o `HtmlParser` sobre `text/html`. Um perito que lesse só o tipo de parsing registraria o media type errado no laudo.
- Q: E quando não há texto mesmo? → A: **O motivo passa a ser derivado do item.** Oferecer três hipóteses de uma vez não é declarar ausência, é não saber em voz alta — e para item decodificado as três eram falsas. O motivo agora sai de `isDecodedData`, do media type, de `isTimedOut` e dos campos de metadado que aquele item tem, nomeando-os. **FR-084 criado.**
- Q: Por que isso é grave e não cosmético? → A: **Escala e classe.** No caso medido são 263.930 itens decodificados e 1.243 chats — as mensagens, que é sobre o que se pergunta numa extração de celular. Um agente que aceita "nenhum texto" conclui que as conversas estão vazias, e essa é a falsa negativa confiante que o FR-047 existe para impedir.

### Session 2026-09-05 — conteúdo que só existe como preview, vindo de campo

Terceira chamada real do mesmo caso: `iped_item_text` respondeu `available: false` com *"Repository not configured. Call configureWritable/ReadOnly() first for: &lt;caminho&gt;"*. Mesmo padrão das sessões anteriores — uma inicialização que a interface faz ao abrir caso e o servidor não fazia.

- Q: O item não tinha conteúdo? → A: **Tinha.** Quando o item lido do índice não tem arquivo próprio nem referência para dentro da evidência, o `IndexItem` lhe dá um `PreviewInputStreamFactory`: os únicos bytes dele estão no `previews.mv.db` do caso. O `UICaseDataLoader` chama `PreviewRepositoryManager.configureReadOnly(moduleDir)` ao abrir — quatro linhas antes do `installCustomSignatures` da sessão anterior — e o servidor nunca chamava. **FR-085 criado.**
- Q: Por que isso apareceu como propriedade da evidência? → A: **Porque a falha surge onde o conteúdo é lido.** O `extractText` capturava tudo e respondia "este item não pôde ser parseado", que é uma afirmação sobre o item. Pior, o remédio mandava tentar `iped_item_content`, que alcança o item pela **mesma** tubulação: no `iped_item_content` a exceção nem era capturada (`catch (IOException)` não pega `IllegalStateException`) e virava erro de protocolo. Falha do servidor passa a ser declarada como tal. **FR-086 criado.**
- Q: Somente-leitura por quê? → A: **Para não tomar o caso do perito.** `configureWritable` trava o H2 com exclusividade; `ACCESS_MODE_DATA=r` admite vários leitores, então o perito segue com o caso aberto na interface. O servidor não processa: não tem o que escrever ali.
- Q: E fechar, é obrigatório? → A: **É, e por dois motivos.** O gerenciador é indexado por pasta e global ao processo, então sem o par o servidor segura o `previews.mv.db` de um caso que nenhuma sessão detém — contradizendo o que o `iped_close_case` promete ("sem deixar trava na pasta"). O contrapeso é a contagem de referências do `CasePool`: libera quando a **última** sessão solta, não qualquer uma.
- Q: Como o teste encontra a classe afetada? → A: **Pela ausência de tamanho, e custou três tentativas.** `hasPreview:true` são 1,7 milhão de itens neste caso, quase todos arquivos comuns com preview renderizado que leem da evidência: amostrar dezenas não achou nenhum afetado e o teste **passou com o conserto removido**. Ampliar para `OR category:"Chats"` piorou — devolveu outra população (plists, bancos de apps de mensagem), nenhum registro de mensagem nos primeiros 75. O que separa está no item: registro decodificado **não tem comprimento próprio**, porque nenhum trecho da evidência é ele. Com `category:"Instant Messages"` + ausência de tamanho, 75 de 75 são da classe afetada, e o controle negativo passa a falhar como deve.
...
- **FR-012**: O sistema MUST retornar, para toda consulta, a contagem total de correspondências independentemente de quantos resultados sejam devolvidos. A contagem MUST ser exata quando a varredura se completar, e MUST vir **declarada como piso** quando o orçamento de tempo a interromper (ver FR-082) — o que o consumidor não pode receber é um número sem saber qual dos dois ele é.
...
- **FR-079**: Um cursor de continuação devolvido MUST avançar: percorrer as páginas de uma consulta MUST visitar cada item uma vez e terminar. Quando o sistema não conseguir produzir um cursor utilizável, MUST omiti-lo em vez de devolver um que não avança — cursor que reinicia em silêncio faz o consumidor reprocessar a mesma página indefinidamente sem sinal de erro, que é falha pior do que a paginação indisponível. **Página parcial não produz cursor utilizável**: ele retomaria depois de uma posição que a varredura não alcançou, e acertos ordenados antes dela ficariam de fora desta página e de todas as seguintes.
- **FR-081**: Quando a operação de consulta receber um marcador, a expressão MUST ser opcional, e sua ausência MUST significar o marcador inteiro. Exigir expressão para listar um marcador obriga o consumidor a inventar uma que signifique "tudo", e a grafia que ele inventa custa a enumeração do dicionário de termos — a superfície não pode criar o custo que depois cobra.
- **FR-082**: Uma página de resultados MUST custar **uma** avaliação da consulta. O total MUST vir da própria coleta, que já conta todo o conjunto; e o orçamento de tempo MUST cobrir tudo o que o sistema faz para responder a página — contagem incluída. Quando o orçamento estourar, o total devolvido MUST ser declarado como **piso** e não como exato: contagem exata obtida fora do orçamento faz o `timeout_ms` pedido pelo consumidor proteger metade do trabalho, que é como uma consulta ampla deixa de responder em vez de responder parcial.
  - O orçamento limita a **varredura**, não a montagem da consulta. Uma expressão cuja expansão é em si caríssima gasta tempo antes de o relógio ser consultado, e o sistema MUST NOT apresentar `timeout_ms` como garantia de tempo de resposta.
  - Expressão que signifique "todo item" MUST ser reconhecida e respondida pelo caminho barato, com o reparo declarado. **Somente** a expressão que é apenas a estrela: adivinhar além disso seria o sistema decidir o que foi pedido.
...
- **FR-083**: O texto devolvido para um item MUST ser o conteúdo dele, nunca a sua forma de origem. Para isso o sistema MUST registrar, no arranque e **antes de existir qualquer parser no processo**, os media types que o IPED declara, do mesmo modo que a interface faz ao abrir caso: é por essa declaração de hierarquia que um tipo atribuído a item decodificado alcança um parser capaz de tratá-lo. Quando ainda assim nenhum parser responder pelo tipo — instalação que antecede o decoder que processou o caso —, o sistema MUST deixar o tipo ser detectado do conteúdo em vez de impor o armazenado, e MUST declarar que o fez. Impor um tipo sem parser seleciona o tratamento genérico de último recurso, que devolve os bytes imprimíveis e **nunca falha**: o defeito chega ao perito como texto plausível, não como erro.
- **FR-084**: Quando não houver texto para um item, o motivo MUST ser derivado do que se sabe daquele item — sua natureza (registro decodificado, diretório), seu media type, se o parsing expirou no processamento, e quais campos de metadado dele carregam conteúdo — e MUST nomear onde o conteúdo está quando ele existir em outro lugar. Enunciar hipóteses alternativas ("pode ser A, B ou C") **não** cumpre a declaração de ausência do FR-022: para a classe mais comum de uma extração de celular as três hipóteses eram falsas enquanto o conteúdo estava nos metadados, e um agente que aceita esse motivo relata as conversas como vazias (FR-047).
- **FR-085**: Ao abrir um caso, o sistema MUST abrir também os repositórios de onde o conteúdo daquele caso é lido, e ao fechá-lo MUST liberá-los. Item decodificado de dentro de um contêiner pode não ter trecho de evidência que seja ele: o conteúdo que o perito vê é o preview que o IPED gravou no caso, e sem esse repositório aberto **não há bytes** para ele. A abertura MUST ser somente-leitura, para que o servidor jamais tome do perito o caso que ele tem aberto na interface, e MUST NOT impedir a abertura do caso quando falhar — um caso sem previews continua consultável. A liberação é parte do contrato de fechar o caso "sem deixar trava na pasta".
- **FR-086**: Falha de inicialização ou de recurso do próprio sistema MUST ser declarada como falha do sistema, nunca como propriedade do item. Um recurso que o servidor não abriu chega ao leitor onde o conteúdo é lido, e responder ali "não há conteúdo" é afirmar sobre a evidência algo que o sistema não verificou — a falsa negativa confiante do FR-047 pela porta dos fundos. A resposta MUST dizer que a falha é do servidor e MUST NOT encaminhar o consumidor para outra ferramenta que dependa do mesmo recurso.
