...
- [ ] T009 Empacotar os binários nativos: `tools/yara-x/win64/yara_x_capi.dll` e `tools/yara-x/linux64/libyara_x_capi.so` — ambos baixados direto do release oficial do upstream (`https://github.com/VirusTotal/yara-x/releases`, assets `libyara-x-capi-vX.Y.Z-x86_64-pc-windows-msvc.zip` / `-x86_64-unknown-linux-gnu.tar.gz`). Substituir o placeholder de T001 e atualizar `tools/yara-x/README.md` com a versão exata empacotada e SHA-256 dos binários. *(Fora deste slice — exige download do release oficial e registro de SHA-256. Diferente do YARA clássico, a Windows DLL aqui é fornecida pelo upstream pré-compilada, sem build manual.)*
