...
### User Story 2 - Abrir e analisar casos pré-existentes sem regressão (Priority: P2)

Um perito abre, no release Java 21, um caso que foi processado por um release anterior (Java 11) e consegue buscar, navegar, filtrar, visualizar itens, usar bookmarks e gerar relatórios normalmente.

**Why this priority**: Casos já periciados precisam continuar acessíveis por anos. Quebrar a leitura de casos existentes inviabilizaria a adoção da nova versão em ambientes operacionais.

**Independent Test**: Abrir um conjunto de casos previamente processados (incluindo casos portáteis) no build Java 21 e validar busca, visualização, bookmarks e relatórios. Entrega valor isolada: garante continuidade operacional.

**Acceptance Scenarios**:

1. **Given** um caso processado por um release Java 11 (índice Lucene, storage, bookmarks, thumbnails), **When** ele é aberto pela UI do release Java 21, **Then** a busca full-text, a navegação, os filtros e a galeria funcionam sem erro e retornam os mesmos resultados.
2. **Given** um caso portátil gerado no build Java 11, **When** aberto no build Java 21, **Then** ele abre e é navegável.
3. **Given** um caso antigo que contém um banco de grafo (Neo4j) gerado antes da migração, **When** o caso é aberto no release Java 21, **Then** o carregamento do caso **não falha**; a aba de grafo degrada de forma controlada, indicando que o grafo precisa ser reconstruído (reprocessamento), sem impedir o restante da análise.
...
- **Caso com grafo Neo4j antigo**: ao abrir um caso cujo banco de grafo foi gerado pelo Neo4j 4.x (formato incompatível com a engine usada no Java 21), o carregamento do caso não pode falhar; a funcionalidade de grafo indica reconstrução necessária em vez de travar.
...
- **FR-004**: Casos criados por releases Java 11 (índice Lucene, bookmarks, storage, thumbnails) MUST abrir e ser pesquisáveis/analisáveis no release Java 21, sem reindexação por parte do usuário.
- **FR-005**: Casos portáteis gerados em releases anteriores MUST permanecer abríveis no release Java 21.
- **FR-006**: A análise de grafo MUST funcionar no Java 21 para casos processados após a migração.
- **FR-007**: Não é requerido preservar bancos de grafo de casos processados antes da migração; abrir tal caso MUST NOT causar falha — a funcionalidade de grafo degrada de forma controlada (indicando reconstrução por reprocessamento).
...
- **Caso processado**: saída do IPED (índice Lucene, storage, bookmarks, thumbnails, eventual graph DB). Deve manter compatibilidade de leitura para dados não-grafo.
...
- **SC-003**: 100% dos casos do conjunto de validação (incluindo casos portáteis) abrem e são pesquisáveis no release Java 21 sem erro.
...
- **Não há requisito de preservar bancos de grafo** de casos processados antes da migração; análise de grafo de casos antigos pode exigir reprocessamento.
...
- O formato de índice de caso (Lucene, linha 9.x) permanece legível; dados não-grafo de casos existentes não exigem reindexação.
...
- **Neo4j embarcado** (hoje 4.4.x) não executa em Java 21 e precisa ser elevado a uma release compatível; este é o subsistema de maior risco e a principal restrição técnica. Como não há requisito de preservar grafos antigos (ver FR-007), a troca de formato de store é aceitável.
