...
> **Base deste branch**: tag **`4.3.1`** do upstream (commit `9d558775a`, 2026-03-28). Esta documentação descreve **a árvore da 4.3.1** — Java 11, Neo4j 4.4.4 embarcado in-process, sem engine YARA. Trabalhos posteriores do fork (migração Java 21, YARA-X, Neo4j out-of-process, Spec Kit) vivem no branch `master` e **não** se aplicam aqui. Ao portar mudanças entre os branches, revalide as versões e caminhos citados antes de copiar texto.
...
Build: **Java 11 LTS + JavaFX**, Maven multi-módulo, distribuído com JRE embarcado + ferramentas externas (Sleuthkit, ImageMagick, Tesseract, LibreOffice, MPlayer, RegRipper, libpff, libesedb, evtxexport, rifiuti2, GraphViz, etc.).

## 2. Estrutura do repositório

```
IPED/
├── pom.xml                                 # Parent POM (versão 4.3.1)
├── README.md, LICENSE.txt, ThirdParty.txt, ReleaseNotes.txt
├── licenses/                                # licenças de terceiros distribuídas
├── .github/workflows/maven.yml              # CI (Ubuntu 22.04; job principal JDK 11 `jdk+fx`, job extra BellSoft JDK 14 Full)
│
...
| Camada | Stack |
|---|---|
| Linguagem | Java 11 LTS (Liberica/BellSoft Full JDK com JavaFX) |
...
| Graph | Neo4j 4.4.4 (engine embarcado **in-process** no `iped-engine`, via `GraphServiceImpl`/`GraphDatabaseService`) |
| Web API | Jersey 2.30.1 + Grizzly + Swagger |
...
| Cache/serial | Caffeine 3.2.2, Zstd-JNI 1.3.3-3, FST 2.57 (`org.nustaq`, serialização do autômato do `RegexTask`) |
...
- **Java 11 LTS** (`maven.compiler.source`/`target = 11`), `UTF-8` por todo o source.
- **Versão**: `4.3.1` no parent; modules herdam.
- **Branch**: este branch parte da tag `4.3.1` do upstream. O tronco de desenvolvimento do fork é `master`.
...
- **Idioma do código**: todo **código novo** (Java, JS, Python, XML, properties) **DEVE** ser comentado e documentado em **inglês** — Javadocs, comentários, mensagens de log, exceções técnicas. Javadocs/comentários PT-BR no código legado **podem permanecer**; ao reescrever substancialmente uma região, traduzir o comentário/Javadoc afetado no mesmo PR. Strings visíveis ao usuário final são localizadas via bundles em `iped-app/resources/localization/` (PT-BR + EN, no mínimo).
...
├── jre/                              # JRE 11.0.13 embutido (artefato maven `java:jre:zip`, Windows)
├── plugins/                          # JARs externos (Stanford CoreNLP, telegram-decoder, java-dbx, ...)
└── iped.exe / bin/IPED-SearchApp.exe # launchers Windows
```

CLI principais (entry points):
- `iped.app.bootstrap.Bootstrap` — processamento de caso (CLI). Flags principais: `-d`/`-data` (datasource), `-o`/`-output` (saída), `-profile` (forensic/pedo/triage/fastmode/blind), `--append`/`--continue`/`--restart`. Lista completa em `iped.engine.CmdLineArgs`.
...
| Graph (Neo4j) | `iped-app/resources/config/conf/GraphConfig.json` |
...
Últimos commits da tag `4.3.1` (top `git log`):
- `#2855` — adiciona o texto transcrito ao HTML report do Telegram; troca concatenação por `StringBuilder`.
- `#2842` — externaliza timeouts para arquivo de configuração (30 s para obter conexão, 10 min para a resposta).
- `#2843` — trata mapeamento LID em chats recuperados do WhatsApp iOS.
- `#2848` — melhora suporte a mapeamento LID (grupos e participantes).
- `#2849` — evita erros ao parsear bancos muito antigos do WhatsApp iOS.
