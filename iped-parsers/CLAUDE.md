...
### Outros
- `APKParser`, `VCardParser`, `OFXParser`, `GenericOLEParser`, `ExternalParser`, `PythonParser`, `OCRParser` (Tesseract via JEP), `TiffPageParser`, `EmptyVideoParser`, `FLVParserWrapper`, `MP4Detector`.
- Pacote `iped.parsers.security/`: `CertificateParser`, `KeystoreParser`, `CryptoAPIBlobParser` (+ `capi/` para blobs DPAPI).
- Pacote `iped.parsers.ufed/`: `UfedChatParser`, `UfedMessageParser`, `UfedEmailParser`, `UfedAccountableParser` + `model/`, `handler/`, `reference/`.
