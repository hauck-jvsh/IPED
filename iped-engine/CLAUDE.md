...
Versão: `4.3.1`. Java 11+. Lucene 9.2.0, Tika 2.4.0, Sleuthkit 4.12.0.p1.
...
- **Tasks**: `HashTaskConfig`, `IndexTaskConfig`, `ParsingTaskConfig`, `SignatureConfig`, `CategoryConfig`, `CarverTaskConfig`, `OCRConfig`, `RegexTaskConfig`, `AudioTranscriptConfig`, `ImageThumbTaskConfig`, `VideoThumbsConfig`, `DocThumbTaskConfig`, `MakePreviewConfig`, `HtmlReportTaskConfig`, `PhotoDNAConfig`, `PhotoDNALookupConfig`, `FaceRecognitionConfig`, `AgeEstimationConfig`, `MinIOConfig`, `ElasticSearchTaskConfig`, `HashDBLookupConfig`, `TempFileTaskConfig`, `SplitLargeBinaryConfig`.
- **UI**: `LocaleConfig`, `SplashScreenConfig`.
- **AI/Graph**: `AIFiltersConfig`, `RemoteImageClassifierConfig`, `NamedEntityTaskConfig`.
- **Export**: `ExportByCategoriesConfig`, `ExportByKeywordsConfig`, `CategoryToExpandConfig`, `ExternalParsersConfig`, `ParsersConfig`.
...
Configuração: não há `Configurable` dedicado — `iped.engine.webapi.Main` lê argumentos de linha de comando (`--port=`, host, `urlToAskSources`) e sobe o Grizzly via `GrizzlyHttpServerFactory`. Default: porta `8080`.
...
| `org.neo4j:neo4j` | 4.4.4 | Graph (engine full Neo4j **embarcado in-process** no próprio `iped-engine`) |
...
| `de.ruedigermoeller:fst` | 2.57 | Serialização rápida (autômato do `RegexTask`) |
| `postgresql:postgresql` | 9.1-901-1.jdbc4 | Driver JDBC legado; sem consumidor no código Java da 4.3.1 (só distribuído — ver `licenses/PostgresqlJDBC.txt`) |
