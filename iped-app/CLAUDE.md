...
Versão `4.3.1`. Java 11 + JavaFX. Empacotamento: `jar`. Main-class no `META-INF/MANIFEST.MF`: `iped.app.bootstrap.Bootstrap`.
...
│   │   └── regex_validators/
│   ├── localization/                    # iped-{desktop,engine,parsers,viewer,geo,utils}-messages, iped-{categories,filters,properties,ai-filters} + locales (pt_BR, es_AR, de_DE, fr_FR, it_IT)
│   ├── plugins/                         # vazio por padrão (drop-in)
│   ├── root/                            # bin/IPED-SearchApp.exe, help/, htmlreport/, iped.exe (launchers versionados no repo)
...
**Busca/filtros**:
- `App.queryComboBox`, `App.filterComboBox` (campos `JComboBox<String>`) — histórico de queries e filtros.
- `FilterManager` — combinação AND/OR/NOT de filtros (+ `ComboFilterer`, package-private no mesmo arquivo).
- `SimilarImagesFilterPanel` / `SimilarImagesFilterActions` + `App.SimilarImagesQueryFilterer` (classe interna).
- `SimilarFacesFilterPanel` / `SimilarFacesFilterActions` + `App.SimilarFacesSearchFilterer` (classe interna).
- `App.SimilarDocumentFilterer`, `App.DuplicatesFilterer`, `App.SearchFilterer` — todas classes internas de `App`.
...
- Renderers por tipo de nó em [`graph/renderers/`](src/main/java/iped/app/graph/renderers): `PersonNodeRenderer`, `PeopleNodeRenderer`, `CompanyNodeRenderer`, `DocumentNodeRenderer`, `EmailNodeRenderer`, `PhoneNodeRenderer`, `CarNodeRenderer`, `MoneyBagNodeRenderer`, `MoneyTransferNodeRenderer`.
...
### Resultado final em `target/release/iped-4.3.1/`

```
├── iped.jar                         (Bootstrap)
├── iped.exe                         (Windows launcher, versionado em resources/root/)
├── bin/IPED-SearchApp.exe           (launcher da UI de análise)
├── jre/                             (JRE 11.0.13 embutido, do artefato maven `java:jre:zip`)
├── lib/
...
| `*Filterer` | Combina filtros (`IResultSetFilterer`/`IQueryFilterer`) | `App.SimilarImagesQueryFilterer`, `App.DuplicatesFilterer`, `DynamicDuplicateFilter`, `CaseSearcherFilter`, `TableHeaderFilterManager` |
...
- **JavaFX** em viewers (HTML, áudio, metadata): `Platform.runLater(...)`. `Platform.setImplicitExit(false)` para não fechar JVM quando última `Stage` fecha.
...
Adicione em `MenuClass` (popup contextual da tabela) ou `App` (menu principal). Implemente `ActionListener` ou `AbstractAction` com chave de localização em `iped-desktop-messages.properties`.
...
1. Adicione a chave no bundle certo em [`iped-app/resources/localization/`](resources/localization) — a UI Swing usa `iped-desktop-messages.properties` (`iped.app.ui.Messages`, `BUNDLE_NAME = "iped-desktop-messages"`); há também `iped-engine-messages`, `iped-parsers-messages`, `iped-viewer-messages`, `iped-geo-messages`, `iped-utils-messages`, `iped-categories`, `iped-filters`, `iped-properties`, `iped-ai-filters`.
2. Replique para `_pt_BR`, `_es_AR`, `_de_DE`, `_fr_FR`, `_it_IT` do mesmo bundle.
3. Use `Messages.getString("MyKey")` — confira qual classe `Messages` a classe que você edita importa; há uma por camada.
...
| Shutdown JavaFX | `Platform.exit` deve esperar o JavaFX terminar antes do `System.exit` — race conditions produzem exceções de `EmbeddedScene`/`GlassScene`. **Problema conhecido na 4.3.1**; corrigido upstream depois (`#2874`/`#2875`). |
...
- **JRE**: `jre/` (Windows) — Java 11 Full FX (artefato `java:jre:11.0.13:zip` do repo iped-maven, descompactado pela execution `unpack-jre`).
