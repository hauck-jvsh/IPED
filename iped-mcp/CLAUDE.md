...
| Texto de item é o conteúdo dele, nunca o fonte | `ContentAccess.extractText` só fixa `Indexer-Content-Type` quando `StandardParser.hasSpecificParser` confirma que há parser para ele. Sem isso o fallback de strings cruas devolvia o HTML do preview como se fosse o texto do chat. `ItemTextTest` fixa |
